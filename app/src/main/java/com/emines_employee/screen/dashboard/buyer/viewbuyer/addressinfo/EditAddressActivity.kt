...
class EditAddressActivity : BaseActivity() {
    ...
    override fun onCreateInit(binding: ViewDataBinding?) {
        ...
        mBind.apply {
            ...
            /*  try {
             *//*     val buyerId = intent.getStringExtra(Constants.DefaultConstant.STRING_KEY).toString()
                if (buyerAddressDetail?.id == null) {
                    mLog("buyerID $buyerId buyerAddressDetail $buyerAddressDetail")
                } else {
                    mLog("buyerID $buyerId buyerAddressDetail $buyerAddressDetail")
                }*//*
            } catch (e: Exception) {
...
*/
            mLog("Buyer ID $buyerID state id $stateId country id $countryId")

            etAInfoType.setOnClickListener {
                dropDownPopup(this@EditAddressActivity,
                    it,
                    ...
                    setOnClickListener {
                        if (etAInfoTypeState.text.toString().isEmpty()) {
                            ...
                            mViewModel.getAddBuyerAddressResponse()
                                .observe(this@EditAddressActivity, saveAddressObserver)
                        }
                    }
