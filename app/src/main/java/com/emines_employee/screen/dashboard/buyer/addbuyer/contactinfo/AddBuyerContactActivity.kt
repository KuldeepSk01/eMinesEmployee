package com.emines_employee.screen.dashboard.buyer.addbuyer.contactinfo
...
import com.emines_employee.databinding.ActivityAddBuyerContactBinding
...
import com.emines_employee.screen.dashboard.buyer.addbuyer.companyinfo.AddBuyerCompanyActivity
...
class AddBuyerContactActivity : BaseActivity() {
    private lateinit var mBind: ActivityAddBuyerContactBinding
    private val mViewModel: BuyersViewModel by inject()
    private var profileImage: String? = null


    override val layoutId = R.layout.activity_add_buyer_contact
    ...
    override fun onCreateInit(binding: ViewDataBinding?) {
        mBind = binding as ActivityAddBuyerContactBinding
        mBind.apply {
            mPref.getBuyerDetail()?.let {
                mBind.apply {
                    etBuyerType.text =
            ...
            etBuyerType.setOnClickListener {
                dropDownPopup(
                    this@AddBuyerContactActivity,
                    it,
                    R.menu.menu_types_of_buyer,
                    object : OnDropDownListener {
                        override fun onDropDownClick(item: String) {
                            etBuyerType.text = item
                        }
                    }).show()
            }

            etBuyerProfileText.setOnClickListener {
                try {
                    CustomDialogs.showSelectProfileBottomSheetDialog(this@AddBuyerContactActivity,
                        object : CustomDialogs.OnBottomSheetClickListener {
                            override fun onImageClick(dialog: BottomSheetDialog) {
                                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                                    ...
                                    if (verifyCameraPermission(this@AddBuyerContactActivity)) {
                                        val gallery = Intent(
                                            Intent.ACTION_PICK,
                                        ...
                                        showSettingsDialog(this@AddBuyerContactActivity)
                                    }
                                }
                                dialog.dismiss()
                            }
            ...
            mBind.apply {
                when (it.msg) {
                    ValidationResult.EMPTY_BUYER_TYPE -> {
                        etBuyerType.error = getString(it.code)
                        etBuyerType.requestFocus()
                    }
                            ...
                            if (IsValidBuyerSellerField) {
                                mPref.getBuyerDetail()?.let {
                                    val addBuyerRes = AddBuyerRequest().apply {
                                        ...
                                        buyer_type = etBuyerType.text.toString()
                                ...
                                mPref.addBuyerDetail(addBuyerRes)
                                launchActivity(AddBuyerCompanyActivity::class.java)
            ...
            when (it.status) {
                ...
                ApiResponse.Status.ERROR -> {
                    ...
                    if (err == PHONE_EXIST || err == EMAIL_EXIST) {
                        ...
                        //launchActivity(BuyerInfoActivity::class.java)
                    }
                }
            }
        }
  ...
  /*  private fun pickImage() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            ...
            val permit = verifyStoragePermission(this)
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            try {
                if (result.resultCode == Activity.RESULT_OK) {
                    ...
                    mBind.ivBuyerInfoProfile.apply {
                        visibility = View.VISIBLE
                        setImageURI(uri)
                    }
                    profileImage = getRealPathFromURI(uri!!, this@AddBuyerContactActivity).toString()
                }
            } catch (e: Exception) {
                profileImage = null
                mBind.ivBuyerInfoProfile.visibility = View.GONE
        ...
        registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            try {
                if (result.resultCode == Activity.RESULT_OK) {
                    ...
                    val path = compressImageFilePath(b, this@AddBuyerContactActivity)

                    if (checkFileSize(File(path))!! < 2.toDouble()) {
                        profileImage = path
                        mBind.ivBuyerInfoProfile.setImageBitmap(b)
